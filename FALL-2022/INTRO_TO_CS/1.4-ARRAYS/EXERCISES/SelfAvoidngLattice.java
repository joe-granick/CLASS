public class SelfAvoidingLattice {
    public static void main(String[] args){
        ...
        while((x > 0 && x < n-1) && (y > 0 && y < n-1))
        {
            ...
            if(r < 0.25)      {if(!blockVisited[x+1][y][z]) x++;}
            else if(r < 0.50) {if(!blockVisited[x-1][y][z]) x--;}  
            else if(r < 0.75) {if(!blockVisited[x][y+1][z]) y++;}
            else if(r < 0.75) {if(!blockVisited[x][y-1][z]) y--;} 
            else if(r < 0.75) {if(!blockVisited[x][y][z+1]) z++;} 
